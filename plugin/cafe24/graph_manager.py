...
class GraphManager():
  ...
  def get_tpid_for_hide_top(self):
    try:
      query =  "select t1.tpid from tpid_mapping_backup as t1, (select t1.job_id, t1.mpid from (select t1.job_id, t2.mpid from job_id_to_site_code as t1 , mpid_top as t2 where concat(10,CAST(t2.site_code AS text),'') = CAST(t1.site_code as text)) as t1 left join (select v1.mpid from (select t1.job_id, t2.mpid from job_id_to_site_code as t1 , mpid_top as t2 where concat(10,CAST(t2.site_code AS text),'') = CAST(t1.site_code as text)) as v1 inner join job_source_view as v2 on v1.mpid = v2.mpid and v1.job_id = v2.job_id) as t2 on t1.mpid = t2.mpid where t2.mpid is NULL) as t2 where t1.job_id = t2.job_id and t1.mpid = t2.mpid and t1.targetsite_url like '%top%';"
      self.gp_cur.execute(query)
      rows = self.gp_cur.fetchall()
      return rows
    except:
